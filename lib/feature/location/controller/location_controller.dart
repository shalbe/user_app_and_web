...
class LocationController extends GetxController implements GetxService {
  ...
  ///
  Future<void> saveAddressAndNavigate(AddressModel address, bool fromSignUp, String? route, bool canRoute, bool isServiceAvailable, {bool fromAddressDialog = false}) async {
    ...
    if(!fromAddressDialog && (getUserAddress() != null && getUserAddress()!.zoneId != null)? !responseModel.zoneIds.contains(getUserAddress()!.zoneId.toString()) : true && Get.find<CartController>().cartList.isNotEmpty) {
      Get.dialog(ConfirmationDialog(
        ...
        onYesPressed: () {
          Get.back();
          _setZoneData(address, fromSignUp, route, canRoute,true, responseModel.zoneIds, previousAddress, isServiceAvailable);
        },
        onNoPressed: () {
          Get.back();
          Get.back();
      ...
      _setZoneData(address, fromSignUp, route, canRoute,false, responseModel.zoneIds, previousAddress, isServiceAvailable);
    }
  }

  void _setZoneData(AddressModel address, bool fromSignUp, String? route, bool canRoute,bool shouldCartDelete, String? zoneIds, AddressModel? previousAddress, bool? isServiceAvailable) {
    if(zoneIds != null){
      Get.find<CartController>().clearCartList();
      address.zoneId = zoneIds;
      autoNavigate(address, fromSignUp, route, canRoute, previousAddress,isServiceAvailable, shouldCartDelete: shouldCartDelete);
    }

  }

  void autoNavigate(AddressModel address, bool fromSignUp, String? route, bool canRoute, AddressModel? previousAddress, bool? isServiceAvailable, {bool shouldCartDelete = false}) async {
    ...
    Get.offAllNamed(RouteHelper.getMainRoute('home', previousAddress: previousAddress));
    if(shouldCartDelete){
